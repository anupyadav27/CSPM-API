...
const ALLOWED_FILTERS = ["tenant_id", "severity", "source", "status", "category", "type", "region", "asset_id"];
...
export const getAllThreatsController = async (req, res) => {
    try {
        ...
        const result = await threatServices().getAllThreats(filters, req.pagination, searchFields, sort);
        ...
        console.info("Error in getAllThreatsController:", error);
...
export const exportThreatsController = async (req, res) => {
    try {
        ...
        const result = await threatServices().getAllThreats(filters, {}, searchFields, sort);

        if (doctype === "xlsx") {
            console.log("Generating Excel...");
            const buffer = await exportToExcel(result.threats, labels);
            ...
            try {
                const buffer = await exportToPDFBuffer(result.threats, labels);
                console.log("PDF buffer size:", buffer.length);
                res.setHeader("Content-Type", "application/pdf");
                res.setHeader("Content-Disposition", "attachment; filename=threats.pdf");
                return res.send(buffer);
            } catch (pdfError) {
        ...
        console.info("Export threats error:", error);
